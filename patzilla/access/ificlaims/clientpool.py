...
def attach_ificlaims_client(event):
    ...
    if request.user and request.user.upstream_credentials and 'ificlaims' in request.user.upstream_credentials:
...
class IFIClaimsClientFactory(object):
    ...
    def __init__(self, api_uri, credentials=None, debug=False):
        ...
        if credentials:
            ...
            message = 'No credentials configured for IFI CLAIMS API.'
            logger.error('IFIClaimsClientFactory: ' + message)
